...
class ScribdDL(object):
    ...
    for f in os.listdir(assets_dir):
        if 'chromedriver' in f:
            DRIVER_PATH = f
    LOAD_TIME = 60  # Stop loading the page after 20 seconds
    START = datetime.now()
    ...
    def start_browser(self):
        ...
        self.driver = webdriver.Chrome(executable_path='C:/Users/frohr/dev/scribdt test/scribd-dl/scribd_dl/assets/chromedriver.exe', options=chrome_options)
    ...
    def _process_url(self, url):
        ...
        retries = 0
        total_pages = None
        while retries < 10:  # Retry up to 2 times to extract the total_pages element
            try:
        ...
        if self.pages != "None":  # If user inserted page range
            try:
